...
apply del_tra_single_letter in HqDel2. apply del_tra_single_letter in HrDel2.
destruct HqDel2 as [o HqDel2]. destruct HrDel2 as [o' HrDel2].
destruct HqDel2 as [HqTra HqLam]. destruct HrDel2 as [HrTra HrLam].
assert (temp := HrLam). rewrite<- H_eq_qr in temp. rewrite HqLam in temp. injection temp as temp.
rewrite<- temp in HrTra. rewrite<- temp in HrLam.
clear temp o'. exists o.
split. 
- apply HqTra.
  ...
  + intros q r H_QMq H_QNr i H_eq_qr. split.
    * split. 
      --  intro H_Mqi_def.
          ...
          { unfold equiv in H_eq_qr. specialize H_eq_qr with (i :: nil). unfold lam in H_eq_qr. rewrite J in H_eq_qr.
          unfold def in H_Mqi_def. destruct H_Mqi_def as [(q', o) H_Mqi_def].
          rewrite H_Mqi_def in H_eq_qr. discriminate H_eq_qr. }
          unfold def. apply J.
      --  intro H_Nri_def. unfold equiv in H_eq_qr.
    ...
    * intros q' r' HqDel HrDel. split.
      ++  (* Obtain the output letter o *)
          ...
          unfold equiv in H_eq_qr. unfold equiv in H_eq_qr.
          intro v. 
          (* Obtain output word V *)
          destruct option_em with (prod Y (word O)) (tra M q' v) as [J|J].
          ** assert (tra M q (i::nil ++ v) = None) as H. 
            { apply (second_half_undefined M (i::nil) v q q' (o :: nil)). apply HqTra. apply J. }
            assert (H0 := H).
            rewrite (lam_tra_undef M q (i::nil ++ v)) in H.
            rewrite H_eq_qr in H.
            rewrite<- lam_tra_undef in H.
            apply (second_half_undefined N (i :: nil) v r r' (o :: nil)) in H.
            { unfold lam. rewrite J. rewrite H. auto. }
            apply HrTra.
          **  destruct J as [(q'', V) J].
              assert (H_q'Tra := J).
...
- intro H_bis. destruct H_bis as [R H_bis].
  ...
  R q r -> Q M q -> Q N r ->  lam M q v = lam N r v) as HH. 
  {
    ...
    - intros q r H_Rqr H_QMq H_QNr.
      ...
      + apply undef_lemma in H_bisDef. assert (tra N r (i :: v) = None). {
          rewrite<- (second_half_undefined N (i::nil) v r r' A). apply Z. 
          destruct L as [L L']. unfold lam in L'. rewrite K in L'. rewrite G in L'. 
          injection L' as L'. rewrite L'. apply G.
        }
        assert (tra M q' v = None) as H0. {
          ...
          unfold lam in Z'. rewrite H0 in Z'. discriminate Z'. apply L.
          apply (reachability M q q' i H_QMq). unfold del. rewrite K. auto.
          apply (reachability N r r' i H_QNr). unfold del. rewrite G. auto.
          apply A.
        }
        rewrite (second_half_undefined M (i::nil) v q q' A) in H0.
        { simpl ((i :: nil) ++ v) in H0. unfold lam. rewrite H. rewrite H0. auto. }
        apply K.
      + destruct Z as [(r'', V) Z]. rewrite<- del_lam_tra_def in Z. destruct Z as [Z0 Z].
        assert (Z' := Z). rewrite<- (IHv q' r') in Z.
        rewrite (lemma_a_1_lambda M (i::nil) v A V q q') in Z. 
        * simpl ((i :: nil) ++ v) in Z.
          rewrite (lemma_a_1_lambda N (i::nil) v A V r r') in Z'. 
          ** simpl ((i :: nil) ++ v) in Z'. rewrite Z. rewrite Z'. auto.
          ** destruct L as [L L']. rewrite<- del_lam_tra_def. split.
             ++ unfold del. rewrite G. auto.
  ...
  intro v. specialize HH with v (q0 M) (q0 N). apply HH. 
  + apply H_Rq0. 
