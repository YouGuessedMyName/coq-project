...
- intros V q q' H J P. assert (K := P). apply first_letter_exi in K.
  ...
  + unfold funcSim in H. destruct H as [H H']. simpl.
    specialize H' with q r a o. assert (G := K''). apply H' in G.
    rewrite<- G. rewrite IH. rewrite K'. simpl. reflexivity. apply J.
  + unfold Q in J. destruct J as [aq J]. unfold Q. exists (aq ++ a :: nil).
